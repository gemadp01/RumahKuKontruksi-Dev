...
export default function MandorTable({ data }) {
    ...
    const sortedMandors = [...filteredMandors].sort((a, b) => {
        switch (sort) {
            ...
            case "newest":
                return new Date(b.tanggal_ditambahkan) - new Date(a.tanggal_ditambahkan);
            case "oldest":
                return new Date(a.tanggal_ditambahkan) - new Date(b.tanggal_ditambahkan);
            default:
                return 0;
        }
    });
    ...
    const handleSubmitForm = (formData) => {
        if (editingMandor) {
            setMandors((prev) =>
                prev.map((m) => (m.id_mandor === editingMandor.id_mandor ? { ...m, ...formData } : m))
            );
        } else {
            const newMandor = {
                ...formData,
